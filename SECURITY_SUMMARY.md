...
## Overview
This PR updates the file upload functionality to use transfer.whalebone.io instead of directly uploading files to Discord. This addresses the requirement for old browsers to upload files through a proxy service.
...
- Key changes:
  ...
  - Increased file size limit from 8MB to 249MB
  - Set 15-minute timeout for upload operations (15 * 60 * 1000 ms)
  - Server proxies files from client to transfer.whalebone.io
  - Discord messages now contain the transfer.whalebone.io URL instead of direct file attachments
...
### Upload Flow
1. Client sends file to server via POST to `/uploadFile`
...
5. Server sends Discord message with the transfer.whalebone.io URL
6. Users can download the file from transfer.whalebone.io
