...
exports.processChannel = async function processChannel(bot, req, res, args, discordID) {
  ...
  try {
    ...
    if (chnl) {
      ...
      const custom_emoji_matches = [...response.matchAll?.(/&lt;(a)?:(\w{2,32}):(\d{17,19})&gt;/g)];
      if (custom_emoji_matches[0] && imagesCookie) custom_emoji_matches.forEach(async match => {
        response = response.replace(match[0], `<img src="/imageProxy/emoji/${match[3]}.${match[1] ? "gif" : "png"}" style="width: 3%;"  alt="emoji">`)
      })
