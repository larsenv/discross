...
function renderScoreboard(events, userTZ) {
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        rowsHtml += render('sports/scoreboard-row', {
            ...
            HOME_NAME: homeName,
            HOME_WEIGHT_END: homeWeightEnd,
            TIME_COLOR: stateType === 'in' ? COLOR_FINAL : color,
...
exports.processSports = async function processSports(req, res) {
    ...
    try {
        ...
        const todayEvents =
            todayResult.status === 'fulfilled' ? todayResult.value?.events || [] : [];
