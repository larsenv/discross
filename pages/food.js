    ...
    return new Promise((resolve, reject) => {
        ...
        if (body) {
            ...
            options.headers['Accept'] =
                'text/plain, application/json, application/json, text/plain, */*';
...
exports.handleGet = async function (bot, req, res, discordID) {
        ...
        const storesHtml = await (async () => {
            try {
                ...
                const trySearch = async (hostname) => {
                    ...
                    return r.status >= 200 && r.status < 300 && r.data && r.data.Stores
                        ? r.data.Stores
                        : [];
                };
  ...
  <a href="/food/menu?store=${encodeURIComponent(s.StoreID || '')}&amp;country=${country}${persistentSuffix}" class="food-btn" style="display:inline-block;margin-top:8px">View Menu</a>
    ...
    if (subpath === 'menu') {
        ...
        const withSelCatMenu = strReplace(
            withStoreIdMenu,
            '{$SELECTED_CATEGORY}',
            escape(category)
        );
            ...
            const rawItemsHtml = productCodes
                .flatMap((code) => {
                    ...
                    const variantPrices = p.Variants?.length
                        ? p.Variants.map((v) => parseFloat((variants[v] || {}).Price || 0)).filter(
                              (v) => v > 0
                          )
                        : [];
  ...
  &#160;<a href="/food/customize?store=${encodeURIComponent(storeId)}&amp;code=${encodeURIComponent(productCode)}&amp;country=${encodeURIComponent(country)}&amp;back=${encodeURIComponent(backUrl)}${persistentSuffix.replace(/&/g, '&amp;')}" class="food-back-link" style="font-size:0.85rem">Change size</a>
                ...
                const normalizedDefaults = Object.fromEntries(
                    Object.entries(defaultOptions).map(([code, portionObj]) => {
                        ...
                        const amt =
                            (portionObj &&
                                (portionObj['1/1'] || portionObj['1/2'] || portionObj['2/4'])) ||
                            (vals && vals[0]) ||
                            '1';
                ...
                const tagDefaults = {
                    ...parseKvStr(defaultToppingsStr),
                    ...parseKvStr(defaultSidesStr),
                };
...
exports.handlePost = async function (bot, req, res, discordID, body) {
    ...
    if (subpath === 'place-order') {
        ...
        const checkoutData = (() => {
            try {
                const cookie = req.headers.cookie || '';
                const checkoutCookieVal = cookie
                    .split('; ')
        ...
        if (
            ...
            console.error(
                ...
                JSON.stringify(
                    checkoutData && {
                        cart: checkoutData.cart ? { items: checkoutData.cart.items } : null,
                    }
                )
            );
            res.writeHead(302, {
                Location: persistRedirect('/food/checkout', [
                    'error=' +
                        encodeURIComponent(
                            'Checkout session expired. Please fill out the form again.'
                        ),
                ]),
            });
            return res.end();
        ...
        if (
            ...
            res.writeHead(302, {
                Location: persistRedirect('/food/checkout', [
                    'error=' +
                        encodeURIComponent(
                            'Missing order details. Please fill out the form again.'
                        ),
                ]),
            });
            return res.end();
        ...
        for (let vStep = 0; vStep < 2; vStep++) {
            const vResult = await dominosRequest(
                ...
                res.writeHead(302, {
                    Location: persistRedirect('/food/checkout', [
                        'error=' +
                            encodeURIComponent('Failed to connect to Dominos. Please try again.'),
                    ]),
                });
                res.end();
                return null;
        ...
        if (priceOuterStatus !== 0 && priceOuterStatus !== 1) {
            ...
            res.writeHead(302, {
                Location: persistRedirect('/food/checkout', [
                    'error=' + encodeURIComponent(priceErrMsg),
                ]),
            });
            return res.end();
        }
