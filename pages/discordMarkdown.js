...
function renderDiscordMarkdown(text, options = {}) {
  ...
  if (rendered.startsWith('<p>') && rendered.endsWith('</p>') && (rendered.match(/<p>/g) || []).length === 1) {
    rendered = rendered.slice(3, -4);
  }

  function resolveNested(str) {
    str = str.replace(/\uE000SPOILER(\d+)\uE001/g, (m, i) => {
      return renderTemplate(tmpl.spoiler, { '{$SPOILER_CONTENT}': md.renderInline(spoilerPlaceholders[parseInt(i, 10)]) });
    });
  ...
  final = final.replace(/\uE000BLOCKQUOTE(\d+)\uE001/g, (m, i) => {
    ...
    const processed = lines.map((l) => l ? resolveNested(md.renderInline(l)) : '\u00A0').join('<br>');
  ...
  final = final.replace(/\uE000BULLETLIST(\d+)\uE001/g, (m, i) => {
    ...
    let html = '<ul>', currentLevel = 0, openTags = 1;
    items.forEach((item) => {
      const level = Math.floor(item.indent / 2);
      while (level > currentLevel) { html += '<ul>'; openTags++; currentLevel++; }
      while (level < currentLevel) { html += '</ul>'; openTags--; currentLevel--; }
      html += `<li>${resolveNested(md.renderInline(item.content))}</li>`;
    });
    while (openTags > 0) { html += '</ul>'; openTags--; }
    return html;
  });
  final = final.replace(/\uE000CODEINLINE(\d+)\uE001/g, (m, i) => `<code>${escapeHtml(codePlaceholders[parseInt(i, 10)].content)}</code>`);
  ...
  final = final.replace(/<a\s+href="https:\/\/discord\.com\/channels\/\d{16,20}\/(\d{16,20})\/(\d{16,20})"/gi, '<a href="/channels/$1/$2"');
  final = final.replace(/\uE000TIMESTAMP(\d+)\uE001/g, (m, i) => {
    const ts = timestampPlaceholders[parseInt(i, 10)];
    const date = new Date(ts.timestamp * 1000);
    const now = new Date();
    const formatTime = (dt) => dt.toLocaleString('en-US', { timeZone: timezone, hour: 'numeric', minute: '2-digit', hour12: true });
    let formatted;
    switch (ts.format) {
      case 'S': formatted = date.toLocaleString('en-US', { timeZone: timezone, month: '2-digit', day: '2-digit', year: '2-digit', hour: 'numeric', minute: '2-digit', second: '2-digit', hour12: true }); break;
      case 'f': formatted = date.toLocaleString('en-US', { timeZone: timezone, year: 'numeric', month: 'long', day: 'numeric' }) + ' at ' + formatTime(date); break;
      case 'F': formatted = date.toLocaleString('en-US', { timeZone: timezone, weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) + ' at ' + formatTime(date); break;
      case 'R':
        ...
        break;
      case 'd': formatted = date.toLocaleString('en-US', { timeZone: timezone, month: '2-digit', day: '2-digit', year: '2-digit' }); break;
      case 'D': formatted = date.toLocaleString('en-US', { timeZone: timezone, year: 'numeric', month: 'long', day: 'numeric' }); break;
      case 't': formatted = formatTime(date); break;
      case 'T': formatted = date.toLocaleString('en-US', { timeZone: timezone, hour: 'numeric', minute: '2-digit', second: '2-digit', hour12: true }); break;
      default: formatted = date.toLocaleString('en-US', { timeZone: timezone, year: 'numeric', month: 'long', day: 'numeric' }) + ' at ' + formatTime(date);
    }
