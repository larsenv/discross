...
/**
 * Process a channel view with shared logic
 * @param {Object} bot - Bot instance
 * @param {Object} req - Request object
 * @param {Object} res - Response object
 * @param {Array} args - URL arguments
 * @param {string} discordID - Discord user ID
 * @param {Object} config - Configuration object for customization
 * @param {string} config.channelTemplate - Path to channel template
 * @param {string} config.messageTemplate - Path to message template
 * @param {string} config.messageForwardedTemplate - Path to forwarded message template
 * @param {string} config.messageMentionedTemplate - Path to mentioned message template
 * @param {string} config.messageForwardedMentionedTemplate - Path to forwarded+mentioned template
 * @param {Function} config.postProcessFinal - Optional callback to modify final HTML before sending
 * @param {Function} config.postProcessResponse - Optional callback to modify response HTML
 */
async function processChannelView(bot, req, res, args, discordID, config) {
  ...
  // Load templates based on config
  const channel_template = fs.readFileSync(config.channelTemplate, 'utf-8');
  const message_template = config.readTemplate 
    ? config.readTemplate(config.messageTemplate)
    : fs.readFileSync(config.messageTemplate, 'utf-8');
  const message_forwarded_template = config.readTemplate
    ? config.readTemplate(config.messageForwardedTemplate)
    : fs.readFileSync(config.messageForwardedTemplate, 'utf-8');
  const message_mentioned_template = config.readTemplate
    ? config.readTemplate(config.messageMentionedTemplate)
    : fs.readFileSync(config.messageMentionedTemplate, 'utf-8');
  const message_forwarded_mentioned_template = config.readTemplate
    ? config.readTemplate(config.messageForwardedMentionedTemplate)
    : fs.readFileSync(config.messageForwardedMentionedTemplate, 'utf-8');
  ...
  try {
    ...
    if (chnl) {
      ...
      const handlemessage = async function (item) { // Save the function to use later in the for loop and to process the last message
        if (lastauthor) { // Only consider the last message if this is not the first
          ...
          if (islastmessage || lastauthor.id != item.author.id || lastauthor.username != item.author.username || item.createdAt - lastdate > 420000) {
            ...
            const authorColor = config.isReplyContext ? "#ffffff" : getMemberColor(lastmember);
      ...
      // Allow post-processing of final HTML before finishing
      if (config.postProcessFinal) {
    ...
    console.log(error)
    if (config.errorHandling === 'redirect') {
...
module.exports = { processChannelView };
